...
from src.config import (
    ...
    ROBERTA_MODEL_NAME as MODEL_NAME,
    ROBERTA_CACHE_DIR, ROBERTA_CHECKPOINT_DIR,
    ROBERTA_N_SAMPLES_SPIKE as N_SAMPLES_SPIKE,
)
...
def load_tokenizer() -> AutoTokenizer:
    ...
    # RoBERTa doesn't have a default pad token if none is set
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
    return tokenizer


def tokenize_samples(tokenizer: AutoTokenizer, texts: list[str]) -> dict:
...
def run_spike() -> None:
    ...
    log_success("Spike complete — environment is ready for Twitter-RoBERTa fine-tuning.")
...
def build_trainer(
    ...
    training_args = TrainingArguments(
        output_dir=output_dir or str(Path(ROBERTA_CHECKPOINT_DIR) / MODEL_NAME.replace("/", "_")),
...
def run_trainer(n_samples: int | None = 500, owner: str = "", notes: str = "") -> Trainer:
    ...
    # Use a sample-size-specific cache dir
    cache_suffix = "full" if n_samples is None else f"n{n_samples}"
    cache_dir = Path(ROBERTA_CACHE_DIR) / cache_suffix
    ...
    log_info(f"Loading {MODEL_NAME} sequence classifier ...")
    # Setting ignore_mismatched_sizes=True safely re-initializes the classification head for our project's label schema
