...
from src.config import (
    ...
    FINBERT_MODEL_NAME as MODEL_NAME,
    FINBERT_CACHE_DIR, FINBERT_CHECKPOINT_DIR,
    FINBERT_N_SAMPLES_SPIKE as N_SAMPLES_SPIKE,
)
...
def run_spike() -> None:
    ...
    log_success("Spike complete — environment is ready for FinBERT fine-tuning.")
...
def build_trainer(
    ...
    training_args = TrainingArguments(
        output_dir=output_dir or str(Path(FINBERT_CHECKPOINT_DIR) / MODEL_NAME.replace("/", "_")),
...
def run_trainer(n_samples: int | None = 500, owner: str = "", notes: str = "") -> Trainer:
    ...
    cache_dir = Path(FINBERT_CACHE_DIR) / cache_suffix
    ...
    # ignore_mismatched_sizes=True safely re-initializes the classification head for our project's label schema
